...
namespace FileWatcher
{
    ...
    public class WatcherExEventArgs
    {
        ...
        RuleEngine objRuleEngine = new RuleEngine();
        #region Constructors
    ...
    /// </summary>
    public class WatcherEx : IDisposable, IFileWatcher
    {
        #region Data Members
        private bool disposed = false;
        private WatcherInfo watcherInfo = null;
        private static WatchersExList watchers = new WatchersExList();
        ...
        /// <summary>
        /// Creates file wather for given foldername
        /// </summary>
        /// <param name="filePath"></param>
        /// <param name="searchCriteria"></param>
        /// <param name="includeSubfolder"></param>
        /// <param name="interval"></param>
        public void CreateFileWatcher(string filePath, string searchCriteria, bool includeSubfolder, int interval)
        {
            WatcherEx objWatcherEx = new WatcherEx(filePath, searchCriteria, includeSubfolder, interval);            
        }
        //--------------------------------------------------------------------------------
        /// <summary>
        ...
        public void DisposeWatchers()
        {
            ...
            for (int i = 0; i < WatcherEx.watchers.Count; i++)
            {
                WatcherEx.watchers[i].Dispose();
            }
            WatcherEx.watchers.Clear();
        }
        ...
        private void Initialize()
        {
            ...
            Debug.WriteLine(string.Format("WatcherEx.Initialize() - {0} watchers created", WatcherEx.watchers.Count));
        }
        ...
        public void Start(int count)
        {
            lock (thisLock)
            {
                ...
                WatcherEx.watchers[count].StartFolderMonitor();

                for (int i = 0; i < WatcherEx.watchers.Count; i++)
                {
                    WatcherEx.watchers[i].EnableRaisingEvents = true;
        ...
        public void Stop(int count)
        {
            lock (thisLock)
            {
                ...
                WatcherEx.watchers[count].StopFolderMonitor();
                for (int i = 0; i < WatcherEx.watchers.Count; i++)
                {
                    WatcherEx.watchers[i].EnableRaisingEvents = false;
